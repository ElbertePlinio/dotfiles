...
if [[ -f "$compaction_extension" ]] \
  ...
  && grep -Fq 'id: "gpt-6-astra"' "$compaction_extension" \
  && grep -Fq 'COMPACTION_THRESHOLD_TOKENS = 240_000' "$compaction_extension"; then
  pass 'Pi model-specific compaction threshold targets GPT-6 Astra at 240k tokens'
else
...
if pi_settings="$(chezmoi "${SRC[@]}" cat "$HOME/.pi/agent/settings.json")"; then
  if jq -e . >/dev/null 2>&1 <<<"$pi_settings"; then
    ...
    jq -e '.defaultProvider == "openai-codex" and .defaultModel == "gpt-6-sol" and .defaultThinkingLevel == "medium"' >/dev/null <<<"$pi_settings" \
      && pass 'Pi canonical bootstrap defaults to GPT-6 Sol' \
      || err 'Pi canonical GPT-6 Sol bootstrap default is missing or misconfigured'
  else
