...
check_live_pi_enabled_models() {
  ...
  if [[ "$STRICT_PREFLIGHT" -eq 1 ]]; then
    ...
    pass 'live Pi enabled models include Grok 4.6 and exclude Luna and Grok 4.5'
  else
    err 'live Pi enabled models must include Grok 4.6 and exclude Luna and Grok 4.5'
  fi
}
