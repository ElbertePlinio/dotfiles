...
5. Promote durable findings: if review or validation surfaced a generalizable, recurring correction (not a one-off bug), add the compact rule to the repo's `AGENTS.md` (or workspace root for cross-repo patterns) in the same PR.
