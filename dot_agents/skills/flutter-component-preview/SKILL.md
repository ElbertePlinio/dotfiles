...
Serve visual lanes from your own shell, `flutter widget-preview start --web-server` or `flutter run -d web-server`, both on loopback, and view the URL in an isolated browser session with a private profile, never the user's browser or real desktop. Wait for a stable frame before each capture so loading frames are not recorded. Chrome cannot shrink to phone widths, so give each viewport its own preview or route that wraps the widget in a `MediaQuery` and `SizedBox` at the requested logical size.
