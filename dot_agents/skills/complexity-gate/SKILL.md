...
description: Use before publishing code by push or PR, for refactoring requests, and for complexity-gate FAIL output. Not a per-edit, per-commit, or code-review step.
---
...
`complexity-gate check --changed` measures changed functions in uncommitted work against `HEAD`. On a clean committed branch it checks nothing and exits 0, so it is not evidence for committed changes. The CLI has no base or commit-range option. For committed work, pass the files changed since the publication base in a Bash script that fails when the base is missing or `git diff` fails:

```bash
set -euo pipefail
base=origin/main
git rev-parse --verify --quiet "$base^{commit}" >/dev/null
git diff -z --name-only --diff-filter=d "$base...HEAD" -- | xargs -0 -r complexity-gate check --
```
...
Fix listed violations without suppressing, renaming, moving, or compressing code to escape the diff or hide branches. Never raise limits in `.complexity-gate.json` to get green. Untouched legacy functions are outside this change's scope.
