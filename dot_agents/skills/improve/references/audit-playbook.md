...
- Shallow modules (use the `codebase-design` skill's vocabulary — module, interface, seam, adapter, depth): interfaces nearly as complex as their implementations; pass-throughs that fail the deletion test (deleting them would concentrate no complexity); single-adapter seams that are pure indirection ("one adapter = hypothetical seam, two = real"); modules whose tests must reach past the interface to assert anything. Report **deepening opportunities** — places where merging shallow modules behind one small interface would gain leverage for callers and locality for maintainers. Preserve "considered and rejected" candidates in the findings so they are not re-audited next run.
