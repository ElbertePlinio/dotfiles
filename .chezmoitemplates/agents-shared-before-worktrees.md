...
- Fable 5 quota is scarce (weekly cap) and the orchestrator usually already runs on it. Never dispatch Fable 5 sub-agent lanes for implementation from a written spec, mechanical work, or anything a cheaper lane can validate — that pays twice for the same capability. If an implementation lane genuinely needs Fable-class judgment and the orchestrator cannot cover it, use Fable at `low` (rarely `medium`) and state why. For judgment, review, and adjudication lanes, shift Fable effort freely between `low` and `high` as the concrete question warrants: a single Fable reviewer at `high` is fine; when dispatching multiple Fable lanes in one wave, prefer `medium` or `low` per lane. `xhigh` Fable lanes only on explicit user request. Grok 4.5 is an allowed writing/implementation lane when Sol/Sonnet don't fit.
