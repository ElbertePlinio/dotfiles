...
- Hard constraints: Grok 4.5 always runs at `high` effort. GLM-5.2 is text-only. If a selected model is unavailable or incompatible, take the closest compatible candidate and report the substitution.
- Fable 5 is weekly-capped. A Fable-run session never dispatches Fable lanes for spec implementation or mechanical work; where Fable-class judgment is genuinely needed beyond its own, use `low` and say why. Other orchestrators may use Fable lanes freely up to `high` for judgment-heavy work. Dispatch prompts state each lane's model selector; a session that cannot determine its own model applies the strict rule.
...
- Implementation from a precise spec routes to cheap lanes by default: Sol first, Sonnet when judgment calls are likely. Review the result at the read tier `local-review` assigns.
...
- Visual browser driving, interactive app testing, and long click/type/verify sessions go to a vision-capable sub-agent (default `openai-codex/gpt-5.6-sol`) that reports in Markdown; the orchestrator reads the report, not the pixels. API and HTTP/DOM-extraction tiers may run in the main session.
...
- `local-review` owns review profiles and counts. Select each reviewer for the concrete failure mode; never assign review roles to fixed models.
