#!/usr/bin/env bash
# PreToolUse gate: before the first Edit/Write of a session inside a git
# repository, block once and require a delegation plan, so lanes and parallel
# subagents are considered before solo implementation begins. Loading the
# model-orchestration skill also satisfies the gate. Claude Code hook schema.
set -euo pipefail
...
if [[ -n "$transcript" && -f "$transcript" ]] && grep -qF "$routing_marker" "$transcript"; then
  exit 0
fi
